...
int main() {
    ...
    while (1) {
        // 拷贝
        BitBlt(nimgDc, 0, 0, ndeskwidth, ndeskheight, ndeskDc, 0, 0, SRCCOPY);
        ...
        if (nullptr == fp) continue; // 文件打开失败，继续下一次循环
        ...
        while (1) {
            ...
            if (ret <= 0) break;
            ret = send(client_socket, (const char*)&buffer, ret, 0);
            if (ret <= 0) {
                fclose(fp);
                break; // 发送数据失败，退出循环
            }
